...
public sealed class OfflineQueueService
{
    ...
    public OfflineQueueService(HttpClient? http = null)
    {
        ...
        var baseUrl = Environment.GetEnvironmentVariable("GGS_SERVER_BASEURL") ?? "https://localhost:5001";
        _ingestUrl = baseUrl.TrimEnd('/') + "/api/ingest/events";
        _maxRows = int.TryParse(Environment.GetEnvironmentVariable("GGS_QUEUE_MAXROWS"), out var m) ? Math.Max(100, m) : 10000;
        _batchSize = int.TryParse(Environment.GetEnvironmentVariable("GGS_QUEUE_BATCHSIZE"), out var b) ? Math.Clamp(b, 1, 500) : 50;
        _http = http ?? new HttpClient { Timeout = TimeSpan.FromSeconds(10) };

        InitDb();
        _timer = new System.Threading.Timer(async _ => await DispatchAsync(), null, TimeSpan.FromSeconds(5), TimeSpan.FromSeconds(5));
    }
    ...
    public async Task DispatchAsync(CancellationToken ct = default)
    {
        try
        {
            ...
            var payload = batch.ConvertAll(e => new { eventId = e.eventId, type = e.type, payload = JsonSerializer.Deserialize<JsonElement>(e.payload), createdUtc = now, client = "agent" });
    ...
    {
        var baseSec = Math.Min(60, attempts * attempts * 5);
