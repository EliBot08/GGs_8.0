...
REM ============================================================================
REM  GGs Desktop Application Launcher
REM  Enterprise-Grade GUI Application Management Script
REM ============================================================================
REM  Features:
REM  - GUI application management
REM  - Window lifecycle management
REM  - Process cleanup and validation
REM  - Launch verification
REM  - Real-time monitoring with uptime counter
REM  - Desktop process tracking
...
color 0B
title GGs Desktop Application Launcher

REM ============================================================================
REM  CONFIGURATION
REM ============================================================================

set "APP_NAME=GGs.Desktop"
set "APP_EXE=GGs.Desktop.exe"
set "PROJECT_PATH=clients\GGs.Desktop"
set "BIN_PATH=clients\GGs.Desktop\bin\Debug\net9.0-windows"
...
echo.
echo ============================================================================
echo  GGs Desktop Application Launcher
echo ============================================================================
echo.
...
call :LOG "============================================================================"
call :LOG "GGs Desktop Application Launcher Started"
call :LOG "============================================================================"
...
if not exist "%PROJECT_PATH%" (
    ...
    echo - Check if the Desktop client project exists
    echo.
...
if not exist "%BIN_PATH%\%APP_EXE%" (
    ...
    echo - Run: dotnet build %PROJECT_PATH%\GGs.Desktop.csproj
    echo - Or use Launch-All-New.bat to build and launch all components
    echo - Ensure .NET 9.0 SDK is installed
    echo.
...
if errorlevel 1 (
    ...
    call :LOG "ERROR: .NET runtime not found"
    echo.
    echo Troubleshooting:
...
echo.
echo [STEP 3/5] Checking for existing instances...
...
if "%ERRORLEVEL%"=="0" (
    ...
    REM Kill existing process gracefully
    ...
    tasklist /FI "IMAGENAME eq %APP_EXE%" 2>NUL | find /I /N "%APP_EXE%">NUL
    if "%ERRORLEVEL%"=="0" (
        echo [WARNING] Forcing termination...
        call :LOG "Forcing termination of existing instance"
    ...
    if "%ERRORLEVEL%"=="0" (
        ...
        echo - Check Task Manager for hung processes
...
echo.
echo [STEP 4/5] Launching %APP_NAME%...
...
if errorlevel 1 (
    ...
    echo - Verify .NET 9.0 Desktop Runtime is installed
    echo - Check Windows Event Viewer for application errors
    echo - Ensure all required DLLs are present in the bin directory
    echo.
    pause
    exit /b 1
)
...
echo.
echo [STEP 5/5] Verifying application startup...
...
if "%ERRORLEVEL%"=="0" (
    ...
    echo - Check the application's error logs in launcher-logs directory
    echo - Verify all dependencies are installed
    echo - Try running the executable directly to see error messages
    echo.
    pause
    exit /b 1
)
...
echo.
echo ============================================================================
echo  Desktop Application Status Monitor
echo ============================================================================
echo.
...
echo Window: Active
echo.
echo The desktop application is now running in its own window.
echo This monitor will track the application status.
echo.
...
if not "%ERRORLEVEL%"=="0" (
    echo.
    echo [INFO] Desktop application has been closed
    call :LOG "Desktop application closed after %COUNTER% seconds"
    echo.
...
<nul set /p "=Runtime: %COUNTER% seconds | Status: RUNNING | Window: Active | Press Ctrl+C to exit     "
echo.
