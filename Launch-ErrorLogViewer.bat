...
:: =============================================================
::  GGs // Error Log Viewer — precision launcher
:: =============================================================
title GGs :: Launch-ErrorLogViewer (DeepOps)
color 0A
...
:: args: [--test] [--duration N] [--skip-dotnet-kill]
:parse
if "%~1"=="" goto parsed
...
:parsed
echo ==============================================================================>>"%LOG_FILE%"
echo [VIEWER] Starting at %DATE% %TIME%>>"%LOG_FILE%"

where dotnet >nul 2>&1 || (echo [ERROR] dotnet CLI not found & exit /b 1)

call :kill_conflicts
call :clean_target "tools\GGs.ErrorLogViewer\GGs.ErrorLogViewer.csproj" "out\viewer"

set "PS_ARGS=-File \"%REPO_ROOT%tools\launcher\Launch-Viewer-New.ps1\" -ForceBuild"
...
if not "%EXIT_CODE%"=="0" (
  echo [FAIL] Viewer launcher failed with exit code %EXIT_CODE%>>"%LOG_FILE%"
  exit /b %EXIT_CODE%
)

echo [OK] Viewer ready.>>"%LOG_FILE%"
exit /b 0

:kill_conflicts
echo [*] Neutralizing conflicting processes...>>"%LOG_FILE%"
for %%P in (GGs.ErrorLogViewer.exe msbuild.exe vstest.console.exe) do (
